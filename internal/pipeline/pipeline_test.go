//nolint:goconst // Test data uses repeated string literals for clarity
package pipeline
...
func TestPipelineDryRun(t *testing.T) {
	...
	for _, file := range summary.Files {
		...
		if strings.HasSuffix(file.Path, "helpers.gen.go") {
...
queries = ["queries.sql"]`
	...
	if err := os.WriteFile(queriesPath, []byte(queryContent), 0o600); err != nil {
